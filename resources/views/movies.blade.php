...
<div class="px-4 md:px-8 py-6 space-y-6 max-w-7xl mx-auto select-none" id="movies-view">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="space-y-1">
            ...
            <p class="text-slate-400 text-sm">Discover latest releases, top-rated masterpieces, and upcoming blockbusters.</p>
        </div>
        
        <!-- Filter Button -->
        <button onclick="openFilters()" class="flex items-center gap-2 px-4 py-2.5 bg-[#1E1E2E] border border-white/5 hover:border-violet-500/20 text-xs font-extrabold text-slate-300 hover:text-white rounded-xl transition duration-200">
    ...
    <div id="active-filters-strip" class="flex flex-wrap items-center gap-2 bg-violet-600/10 border border-violet-500/10 p-3 rounded-2xl hidden">
        <span class="text-[11px] font-extrabold uppercase tracking-wider text-violet-400 mr-1">Active:</span>
        <div id="active-chips" class="flex flex-wrap gap-1.5"></div>
        <button onclick="resetFilters(true)" class="text-[11px] font-extrabold text-violet-400 hover:text-violet-300 transition ml-auto">Clear All</button>
    </div>

    <!-- Tabs Navigation -->
    <div class="border-b border-[#1E1E2E] flex gap-4 overflow-x-auto no-scrollbar scroll-smooth py-1" id="movie-tabs-container"></div>

    <!-- Movies Grid -->
    <div class="space-y-6">
        <div id="movies-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
        ...
        <div id="empty-state" class="hidden text-center py-20 space-y-2 animate-fadeIn">
            ...
            <h3 class="text-md font-bold text-white">No Movies Found</h3>
...
<div id="filter-drawer" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex justify-end hidden select-none" onclick="handleBackdropClick(event)">
    <div id="filter-drawer-content" class="w-full max-w-md bg-[#121220] h-full flex flex-col border-l border-white/5 transform translate-x-full transition-transform duration-300">
        ...
        <div class="px-6 py-5 border-b border-[#1E1E2E] flex justify-between items-center bg-[#1E1E2E]/20">
            <div>
                <h3 class="text-lg font-extrabold text-white">Refine Movies</h3>
        ...
        <div class="p-6 border-t border-[#1E1E2E] flex gap-3 bg-[#1E1E2E]/10">
            ...
            <button onclick="applyFilters()" class="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white text-xs font-bold shadow-lg shadow-violet-500/20 hover:from-violet-500 hover:to-fuchsia-500 transition">
                Apply Filters
            </button>
        </div>
    </div>
...
<script>
    ...
    const movieTabs = [
        ...
        { label: '🆕 New',       sortBy: 'release_date.desc', genreId: null,  trending: false },
        { label: '👊 Action',    sortBy: 'popularity.desc',   genreId: 28,    trending: false },
        { label: '😂 Comedy',    sortBy: 'popularity.desc',   genreId: 35,    trending: false },
        { label: '😱 Horror',    sortBy: 'popularity.desc',   genreId: 27,    trending: false },
        { label: '🌌 Sci-Fi',    sortBy: 'popularity.desc',   genreId: 878,   trending: false },
        { label: '🎭 Drama',     sortBy: 'popularity.desc',   genreId: 18,    trending: false },
        { label: '🪄 Fantasy',   sortBy: 'popularity.desc',   genreId: 14,    trending: false },
        { label: '🔪 Thriller',  sortBy: 'popularity.desc',   genreId: 53,    trending: false },
        { label: '💗 Romance',   sortBy: 'popularity.desc',   genreId: 10749, trending: false },
        { label: '🗺️ Adventure', sortBy: 'popularity.desc',   genreId: 12,    trending: false }
    ];
    ...
    function renderTabs() {
        const container = document.getElementById("movie-tabs-container");
        container.innerHTML = movieTabs.map((tab, idx) => `
    ...
    function switchTab(idx) {
        ...
        document.getElementById(`tab-btn-${idx}`).className = "tab-btn pb-3 text-sm font-semibold transition-all whitespace-nowrap border-b-2 border-violet-500 text-white font-extrabold";
    ...
    async function loadTabContent(tabIdx, page) {
        ...
        if (moviesCache[cacheKey]) {
            ...
            if (page === 1) {
                ...
                moviesList = [...moviesList, ...cached.items];
        ...
        if (tab.trending && isFiltersDefault()) {
            url = '/api/tmdb/trending/movie/week';
            params = { page: page };
        } else {
        ...
        try {
            ...
            if (page === 1) {
                ...
                moviesList = [...moviesList, ...items];
                appendItemsToGrid(items);
                isLoadingMore = false;
                document.getElementById("load-more-indicator").classList.add("hidden");
            }
        } catch (err) {
            console.error("Error loading movies page content:", err);
            isLoading = false;
            isLoadingMore = false;
            document.getElementById("load-more-indicator").classList.add("hidden");
        }
    ...
    function buildCardHtml(item) {
        ...
        return `
            <a href="${detailsUrl}" class="group flex flex-col gap-2 relative">
                <div class="relative aspect-[2/3] rounded-2xl overflow-hidden bg-[#1E1E2E] border border-white/5 transition duration-300 group-hover:scale-[1.03] group-hover:shadow-xl group-hover:shadow-[#0984E3]/10">
                ...
                <div class="px-1 space-y-0.5">
                    <h3 class="text-sm font-bold text-white group-hover:text-[#0984E3] transition truncate w-full">${item.title}</h3>
    ...
    function updateActiveFiltersStrip() {
        ...
        if (activeFilters.genre !== 'All' && movieTabs[currentTabIndex].genreId === null) {
            activeCount++;
