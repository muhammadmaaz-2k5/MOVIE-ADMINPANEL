...
@section('title', 'TV Shows Manager — ENGORA Admin')
...
<div class="px-6 py-8 max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">TV Shows Manager</h1>
            <p class="text-slate-400 text-sm mt-1">Create duplicate or customized entries for any TMDB TV Show ID with local titles, languages, and custom streams.</p>
        </div>
    ...
    <div class="glass p-4 rounded-2xl flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input id="search-input" type="text" placeholder="Search custom TV library..." oninput="filterMovies()" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl pl-9 pr-4 py-2.5 placeholder-slate-500 focus:outline-none focus:border-violet-500/40 transition"/>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody id="movies-table-body" class="divide-y divide-white/5 text-slate-200">
                    <tr><td colspan="6" class="px-5 py-12 text-center text-slate-500 animate-pulse">Loading customized library...</td></tr>
                </tbody>
            </table>
        </div>
        <!-- Pagination -->
...
<div id="movie-modal" class="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center hidden" onclick="closeMovieModal(event)">
    <div id="movie-modal-panel" class="w-full max-w-2xl bg-[#121220] rounded-3xl border border-white/8 shadow-2xl overflow-hidden mx-4 max-h-[90vh] overflow-y-auto scrollbar-thin">
        ...
        <div class="px-6 pt-6 pb-4 border-b border-white/5 flex justify-between items-center sticky top-0 bg-[#121220] z-10">
            <h3 id="modal-title" class="text-lg font-extrabold text-white">Create Custom TV Show</h3>
        ...
        <div id="tmdb-search-section" class="px-6 py-4 border-b border-white/5 space-y-3">
            ...
            <div class="relative">
                <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                <input id="tmdb-search" type="text" placeholder="Type TV Show title to import metadata..." oninput="searchTmdb()" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl pl-9 pr-4 py-2.5 placeholder-slate-500 focus:outline-none focus:border-violet-500/40 transition"/>
            </div>
        ...
        <form id="movie-form" onsubmit="submitMovieForm(event)" class="px-6 py-5 space-y-4">
            ...
            <input type="hidden" id="form-type" value="tv"/>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input id="form-tmdb-id" type="number" placeholder="e.g. 94605 (optional)" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                </div>

                <!-- Custom Title -->
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Custom Title *</label>
                    <input id="form-title" type="text" required placeholder="e.g. Arcane [Hindi]" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                    <input id="form-genre-ids" type="hidden" value="[]"/>
                </div>
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Episode Runtime</label>
                    <input id="form-runtime" type="text" placeholder="45 min" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                </div>
                ...
                <div class="flex items-center gap-3 sm:col-span-2 pt-2">
                    ...
                    <label for="form-is-active" class="text-xs font-semibold text-slate-300 uppercase tracking-wider select-none cursor-pointer">Publish Custom TV Show</label>
                </div>

                <!-- Midnight 18+ VIP Toggle Card -->
                <div class="sm:col-span-2 p-4 rounded-2xl bg-gradient-to-r from-[#FF1A75]/10 via-[#9D4EDD]/10 to-transparent border border-[#FF1A75]/25 space-y-3">
            ...
            <div class="border-t border-white/5 pt-5 flex gap-3">
                <button type="submit" id="submit-btn" class="flex-1 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-bold py-3 rounded-2xl hover:from-violet-500 hover:to-fuchsia-500 transition shadow-lg shadow-violet-500/20 text-sm">Save Custom TV Show</button>
        ...
        <div class="px-6 py-4 border-b border-white/5 bg-white/1">
            ...
            <form id="stream-form" onsubmit="submitStreamForm(event)" class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3" id="tv-fields">
                    <input id="stream-season" type="number" required placeholder="Season Number" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-xs rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                    <input id="stream-episode" type="number" required placeholder="Episode Number" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-xs rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                </div>
...
<div id="delete-modal" class="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center hidden">
    <div class="w-full max-w-sm bg-[#121220] rounded-3xl border border-white/8 shadow-2xl p-6 mx-4 space-y-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-base font-extrabold text-white">Delete customized TV show?</h3>
...
let activeMovieTypeForStreams = 'tv';
...
async function loadCustomLibrary(page = 1) {
    ...
    let url = `/admin/api/custom-movies?page=${page}&type=tv`;
    ...
    try {
        ...
        document.getElementById('movies-table-body').innerHTML = `<tr><td colspan="6" class="px-5 py-12 text-center text-rose-455 text-xs">Failed to load customized library. Check console or try again.</td></tr>`;
    }
}
...
function renderTable(movies) {
    ...
    if (movies.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="px-5 py-16 text-center text-slate-500 text-sm">
            <div class="flex flex-col items-center gap-3">
                ...
                <span>No custom TV shows created yet. Add one to get started.</span>
            </div>
        </td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = movies.map(movie => {
        ...
        return `<tr class="hover:bg-white/1 transition group">
            <td class="px-5 py-3.5">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-[10px] text-slate-500 mt-0.5">${movie.tmdb_id ? 'TMDB: ' + movie.tmdb_id : '<span class="text-violet-400/80 font-medium">Manual Custom</span>'} · Year: ${movie.year || '—'}</p>
                    </div>
                </div>
            </td>
            ...
            <td class="px-5 py-3.5">
                <button onclick="openStreamsDrawer(${movie.id}, '${movie.title.replace(/'/g, "\\'")}', 'tv')" 
            ...
            <td class="px-5 py-3.5">
                <div class="flex gap-2">
                    ...
                    <button onclick="openDeleteModal(${movie.id})" class="p-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 transition" title="Delete Customized TV Show">
...
function searchTmdb() {
    ...
    tmdbSearchTimer = setTimeout(async () => {
        try {
            const data = await fetch(`/api/tmdb/search/tv?query=${encodeURIComponent(q)}`).then(r => r.json());
            ...
            container.innerHTML = results.map(r => {
                ...
                return `<button type="button" onclick='selectTmdbContent(${JSON.stringify({id:r.id,type:"tv",title:title,year:year,poster:r.poster_path||"",backdrop:r.backdrop_path||"",rating:r.vote_average||0.0,overview:r.overview||"",genre_ids:r.genre_ids||[],original_language:r.original_language||"en"}).replace(/'/g,"\\'")})'
...
function selectTmdbContent(c) {
    ...
    document.getElementById('form-genre-ids').value     = JSON.stringify(c.genre_ids || []);
...
function openAddModal() {
    document.getElementById('modal-title').innerText = 'Create Custom TV Show';
    document.getElementById('movie-form').reset();
    document.getElementById('form-id').value = '';
    document.getElementById('form-type').value = 'tv';
...
function openEditModal(movie) {
    document.getElementById('modal-title').innerText = 'Edit Custom TV Show';
...
async function submitMovieForm(event) {
    ...
    try {
        ...
        showToast(id ? 'TV Show updated successfully!' : 'Custom TV Show created!', 'success');
    } catch(e) {
        showToast('Error saving: ' + e.message, 'error');
    } finally {
        btn.disabled = false;
        btn.innerText = 'Save Custom TV Show';
    }
}
...
async function confirmDelete() {
    ...
    try {
        ...
        loadCustomLibrary(currentPage);
        showToast('Custom TV Show deleted.', 'success');
    } catch(e) {
...
async function openStreamsDrawer(id, title, type) {
    ...
    activeMovieTypeForStreams = 'tv';
    document.getElementById('drawer-movie-title').innerText = title;
    document.getElementById('drawer-movie-meta').innerText = 'Add video player servers for this customized TV series';
...
async function loadMovieStreams() {
    ...
    try {
        ...
        list.innerHTML = streams.map(s => {
            const metaInfo = `<span class="px-1.5 py-0.5 bg-violet-600/10 text-violet-400 border border-violet-500/20 text-[9px] font-bold rounded">S${s.season_number} E${s.episode_number}</span>`;
            return `
                ...
                <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5">
                            ${metaInfo}
                            <span class="text-[9px] text-slate-550 truncate font-mono">${s.stream_url}</span>
                        </div>
                    </div>
                </div>
...
async function submitStreamForm(event) {
    ...
    const payload = {
        ...
        season_number:  parseInt(document.getElementById('stream-season').value),
        episode_number: parseInt(document.getElementById('stream-episode').value),
    };
