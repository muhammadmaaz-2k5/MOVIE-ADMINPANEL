...
<div class="px-6 py-8 max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Movie Manager</h1>
            <p class="text-slate-400 text-sm mt-1">Create duplicate or customized entries for any TMDB ID with local titles, languages, and custom streams.</p>
        </div>
    ...
    <div class="glass p-4 rounded-2xl flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input id="search-input" type="text" placeholder="Search custom library..." oninput="filterMovies()" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl pl-9 pr-4 py-2.5 placeholder-slate-500 focus:outline-none focus:border-violet-500/40 transition"/>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody id="movies-table-body" class="divide-y divide-white/5 text-slate-200">
                    <tr><td colspan="7" class="px-5 py-12 text-center text-slate-500 animate-pulse">Loading customized library...</td></tr>
                </tbody>
            </table>
        </div>
        <!-- Pagination -->
...
<div id="movie-modal" class="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center hidden" onclick="closeMovieModal(event)">
    <div id="movie-modal-panel" class="w-full max-w-2xl bg-[#121220] rounded-3xl border border-white/8 shadow-2xl overflow-hidden mx-4 max-h-[90vh] overflow-y-auto scrollbar-thin">
        ...
        <div class="px-6 pt-6 pb-4 border-b border-white/5 flex justify-between items-center sticky top-0 bg-[#121220] z-10">
            <h3 id="modal-title" class="text-lg font-extrabold text-white">Create Custom Movie</h3>
        ...
        <div id="tmdb-search-section" class="px-6 py-4 border-b border-white/5 space-y-3">
            ...
            <div class="relative">
                <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                <input id="tmdb-search" type="text" placeholder="Type title to import metadata..." oninput="searchTmdb()" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl pl-9 pr-4 py-2.5 placeholder-slate-500 focus:outline-none focus:border-violet-500/40 transition"/>
            </div>
        ...
        <form id="movie-form" onsubmit="submitMovieForm(event)" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="form-type" required class="w-full bg-[#1E1E2E] border border-white/5 text-slate-350 text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition">
                ...
                <div class="space-y-1.5 sm:col-span-2">
                    ...
                    <input id="form-title" type="text" required placeholder="e.g. Mission Impossible [Hindi]" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                ...
                <div class="space-y-1.5">
                    ...
                    <input id="form-runtime" type="text" placeholder="120 min" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                </div>

                <!-- Rating -->
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Rating</label>
                    <input id="form-rating" type="number" step="0.1" max="10" placeholder="7.8" class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-violet-500/40 transition"/>
                </div>
            </div>

            <!-- Overview -->
            <div class="space-y-1.5">
                <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Overview Description</label>
                <textarea id="form-overview" rows="3" placeholder="Enter custom overview storyline..." class="w-full bg-[#1E1E2E] border border-white/5 text-white text-sm rounded-xl p-4 focus:outline-none focus:border-violet-500/40 transition resize-none"></textarea>
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="button" onclick="closeMovieModal()" class="px-5 py-3 bg-[#1E1E2E] border border-white/5 text-slate-300 font-bold rounded-2xl hover:bg-white/5 hover:text-white transition text-sm">
                    Cancel
                </button>
            </div>
        </form>
...
══════════════════════════════════════════════════════ -->
<div id="streams-drawer" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex justify-end hidden" onclick="handleDrawerBackdropClick(event)">
    <div id="streams-drawer-content" class="w-full max-w-xl bg-[#121220] h-full flex flex-col border-l border-white/5 transform translate-x-full transition-transform duration-300">
        <!-- Header -->
        <div class="px-6 py-5 border-b border-white/5 flex justify-between items-center bg-[#1E1E2E]/20">
            <div>
...
<div id="delete-modal" class="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center hidden">
    <div class="w-full max-w-sm bg-[#121220] rounded-3xl border border-white/8 shadow-2xl p-6 mx-4 space-y-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-base font-extrabold text-white">Delete customized movie?</h3>
...
async function loadCustomLibrary(page = 1) {
    ...
    let url = `/admin/api/custom-movies?page=${page}`;
    ...
    try {
        ...
        document.getElementById('movies-table-body').innerHTML = `<tr><td colspan="7" class="px-5 py-12 text-center text-rose-450 text-xs">Failed to load customized library. Check console or try again.</td></tr>`;
    }
}
    ...
    if (movies.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-16 text-center text-slate-500 text-sm">
            <div class="flex flex-col items-center gap-3">
                ...
                <span>No custom movies created yet. Add one to get started.</span>
            </div>
        </td></tr>`;
        return;
    }
        ...
        return `<tr class="hover:bg-white/1 transition group">
            ...
            <td class="px-5 py-3.5">
                <div class="flex gap-2">
                    ...
                    <button onclick="openDeleteModal(${movie.id})" class="p-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 transition" title="Delete Customized Movie">
...
function updatePagination(data) {
    ...
    document.getElementById('pagination-info').innerText = `Page ${data.current_page} of ${data.last_page} — ${data.total} customized entries`;
    ...
    tmdbSearchTimer = setTimeout(async () => {
        try {
            ...
            container.innerHTML = results.map(r => {
                ...
                return `<button type="button" onclick='selectTmdbContent(${JSON.stringify({id:r.id,type:r.media_type,title:title,year:year,poster:r.poster_path||"",backdrop:r.backdrop_path||"",overview:r.overview||"",rating:r.vote_average||0,genre_ids:r.genre_ids||[],original_language:r.original_language}).replace(/'/g,"\\'")})'
                    class="flex items-center gap-3 w-full p-2 rounded-xl hover:bg-white/5 text-left border border-white/0 hover:border-violet-500/20">
                    <img src="${poster}" class="w-8 h-11 rounded-lg object-cover bg-[#1E1E2E] flex-shrink-0"/>
...
function openEditModal(movie) {
    document.getElementById('modal-title').innerText = 'Edit Custom Movie';
...
async function submitMovieForm(event) {
    ...
    try {
        ...
        showToast(id ? 'Movie updated successfully!' : 'Custom Movie created!', 'success');
    } catch(e) {
        showToast('Error saving: ' + e.message, 'error');
    } finally {
        btn.disabled = false;
        btn.innerText = 'Save Custom Movie';
    }
}
...
async function confirmDelete() {
    ...
    try {
        ...
        loadCustomLibrary(currentPage);
        showToast('Custom Movie deleted.', 'success');
    } catch(e) {
...
async function openStreamsDrawer(id, title, type) {
    ...
    document.getElementById('drawer-movie-meta').innerText = `Add video player servers for this customized ${type === 'movie' ? 'movie' : 'TV series'}`;
                ...
                <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5">
                            ${metaInfo}
                            <span class="text-[9px] text-slate-500 truncate font-mono">${s.stream_url}</span>
                        </div>
                    </div>
                </div>
